...
import { ShdwDrive, type StorageAccountResponse, type CreateStorageResponse, type ShadowUploadResponse} from "@shadow-drive/sdk";
...
export async function getStorageAccounts(connection: Connection, wallet: any): Promise<StorageAccountResponse[]> {
    const drive = await new ShdwDrive(forcedConnection, wallet).init();
