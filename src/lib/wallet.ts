...
export const walletProgramConnection = (
    ...
    return derived([walletStore, workSpace], ([$walletStore, $workSpace]) => {
      return {
        ...
        metaplex: $workSpace?.provider?.connection && $walletStore.wallet ? Metaplex.make($workSpace.provider.connection).use(walletAdapterIdentity($walletStore.wallet)) : null
      };
    });
  };