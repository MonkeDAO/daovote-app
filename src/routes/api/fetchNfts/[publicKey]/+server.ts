...
export const GET: RequestHandler = async (request) => {
    ...
    for (var nftRaw of nftsRaw) {
      const res = await fetch(nftRaw.uri).catch((err) => { console.log('bad metadata uri skipping', err)});
      if(res) {
        const data = await res.json().catch((err) => { console.log('bad Json skipping', err)});
        if (data) {
          nfts.push({
